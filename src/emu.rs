...
impl Emu {
    ...
    // after setting the maps, init will prepare the environment specially memory layout.
    ...
    fn init_mem32(&mut self) {
        ...
        if !kernel32.load("kernel32.bin") {
            println!("cannot find the maps files, use  emu.set_maps_folder()");
            std::process::exit(1);
        }
    ...
    // set the path of a binary shellcode or PE
    ...
    fn memory_operand_to_address(&mut self, operand:&str) -> u64 {
    ...
    // run until selected address, on that address will stop the emulation. 
    // The addres is not going to be emulated.
    // put zero to emulate totally.
