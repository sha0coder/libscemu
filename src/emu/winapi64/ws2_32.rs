...
fn setsockopt(emu:&mut emu::Emu) {
    ...
    let optlen = emu.maps.read_qword(emu.regs.get_esp()).expect("ws2_32!setsockopt: error reading optlen");
...
fn getsockopt(emu:&mut emu::Emu) {
    ...
    let optname = emu.regs.r8;
    let optval = emu.regs.r9;
    let optlen = emu.maps.read_qword(emu.regs.get_esp()).expect("ws2_32!getsockopt: error reading optlen") as u64;
...
fn WsaAccept(emu:&mut emu::Emu) {
    ...
    let callback = emu.maps.read_qword(emu.regs.get_esp()).expect("ws2_32!WsaAccept: error reading callback") as u64;
