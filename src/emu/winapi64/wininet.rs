...
pub fn InternetOpenA(emu:&mut emu::Emu) {
    ...
    let flags =  emu.maps.read_qword(emu.regs.rsp).expect("wininet!InternetOpenA  cannot read flags");
...
pub fn InternetOpenW(emu:&mut emu::Emu) {
    ...
    let proxy_ptr = emu.regs.r8;
    let proxybypass_ptr = emu.regs.r9;
    let flags =  emu.maps.read_qword(emu.regs.rsp).expect("wininet!InternetOpenW  cannot read flags");
...
pub fn InternetConnectA(emu:&mut emu::Emu) {
    ...
    let passw_ptr = emu.maps.read_qword(emu.regs.rsp).expect("wininet!InternetConnectA cannot read passw_ptr") as u64;
    let service = emu.maps.read_qword(emu.regs.rsp+8).expect("wininet!InternetConnectA cannot read service");
    let flags  = emu.maps.read_qword(emu.regs.rsp+16).expect("wininet!InternetConnectA cannot read flags");
    let ctx  = emu.maps.read_qword(emu.regs.rsp+24).expect("wininet!InternetConnectA cannot read ctx");
...
pub fn InternetConnectW(emu:&mut emu::Emu) {
    ...
    let port = emu.regs.r8;
    let login_ptr = emu.regs.r9;
    let passw_ptr = emu.maps.read_qword(emu.regs.rsp).expect("wininet!InternetConnectW cannot read passw_ptr") as u64;
    let service = emu.maps.read_qword(emu.regs.rsp+8).expect("wininet!InternetConnectW cannot read service");
    let flags  = emu.maps.read_qword(emu.regs.rsp+16).expect("wininet!InternetConnectW cannot read flags");
    let ctx  = emu.maps.read_qword(emu.regs.rsp+24).expect("wininet!InternetConnectW cannot read ctx");
...
fn HttpOpenRequestA(emu:&mut emu::Emu) {
    ...
    let referrer_ptr = emu.maps.read_qword(emu.regs.rsp).expect("wininet!HttpOpenRequestA cannot read referrer_ptr") as u64;
    let access_ptr = emu.maps.read_qword(emu.regs.rsp+8).expect("wininet!HttpOpenRequestA cannot read access_ptr") as u64;
    let flags = emu.maps.read_qword(emu.regs.rsp+16).expect("wininet!HttpOpenRequestA cannot read flags") as u64;
    let ctx = emu.maps.read_qword(emu.regs.rsp+24).expect("wininet!HttpOpenRequestA cannot read ctx");
...
fn HttpOpenRequestW(emu:&mut emu::Emu) {
    ...
    let path_ptr = emu.regs.r8;
    let version_ptr = emu.regs.r9;
    let referrer_ptr = emu.maps.read_qword(emu.regs.rsp).expect("wininet!HttpOpenRequestW cannot read referrer_ptr") as u64;
    let access_ptr = emu.maps.read_qword(emu.regs.rsp+8).expect("wininet!HttpOpenRequestW cannot read access_ptr") as u64;
    let flags = emu.maps.read_qword(emu.regs.rsp+16).expect("wininet!HttpOpenRequestW cannot read flags") as u64;
    let ctx = emu.maps.read_qword(emu.regs.rsp+24).expect("wininet!HttpOpenRequestW cannot read ctx");
...
fn InternetSetOptionA(emu:&mut emu::Emu) {
    ...
    println!("{}** {} wininet!InternetSetOptionA option: 0x{:x} buff: {{{}}} {} {}", emu.colors.light_red, emu.pos, option, 
             buffer_content, sbuff, emu.colors.nc);
...
fn InternetSetOptionW(emu:&mut emu::Emu) {
    ...
    println!("{}** {} wininet!InternetSetOptionW option: 0x{:x} buff: {{{}}} {} {}", emu.colors.light_red, emu.pos, option, 
             buffer_content, sbuff, emu.colors.nc);
...
fn HttpSendRequestA(emu:&mut emu::Emu) {
    ...
    let opt_len = emu.maps.read_qword(emu.regs.rsp).expect("wininet!HttpSendRequestA cannot read opt_len");
...
fn HttpSendRequestW(emu:&mut emu::Emu) {
    ...
    let hdrs_len = emu.regs.r8;
    let opt_ptr = emu.regs.r9;
    let opt_len = emu.maps.read_qword(emu.regs.rsp).expect("wininet!HttpSendRequestW cannot read opt_len");
...
fn InternetReadFile(emu:&mut emu::Emu) {
    ...
    println!("{}** {} wininet!InternetReadFile sz: {} buff: 0x{:x} {}", emu.colors.light_red, emu.pos, 
             bytes_to_read, buff_ptr, emu.colors.nc);
...
fn InternetReadFileExA(emu:&mut emu::Emu) {
    ...
    println!("{}** {} wininet!InternetReadFileExA buff: 0x{:x} {}", emu.colors.light_red, emu.pos, 
             buff_ptr, emu.colors.nc);
...
fn InternetReadFileExW(emu:&mut emu::Emu) {
    ...
    println!("{}** {} wininet!InternetReadFileExW buff: 0x{:x} {}", emu.colors.light_red, emu.pos, 
             buff_ptr, emu.colors.nc);
