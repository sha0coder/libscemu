...
fn CryptStringToBinaryA(emu:&mut emu::Emu) {
    let string = emu.maps.read_dword(emu.regs.get_esp())                                                                                                                                         
        .expect("crypt32!CryptStringToBinaryA error getting flags param");
    let num_chars = emu.maps.read_dword(emu.regs.get_esp()+4)
        .expect("crypt32!PCryptStringToBinaryA error getting addr param");
    let flags = emu.maps.read_dword(emu.regs.get_esp()+8)                                                                                                                                         
        .expect("crypt32!CryptStringToBinaryA error getting flags param");
    let ptr = emu.maps.read_dword(emu.regs.get_esp()+12)
        .expect("crypt32!PCryptStringToBinaryA error getting addr param");
    let inout_sz = emu.maps.read_dword(emu.regs.get_esp()+16)                                                                                                                                         
        .expect("crypt32!CryptStringToBinaryA error getting flags param");
    let skip = emu.maps.read_dword(emu.regs.get_esp()+20)
        .expect("crypt32!PCryptStringToBinaryA error getting addr param");
    let out_flags = emu.maps.read_dword(emu.regs.get_esp()+24)                                                                                                                                         
        .expect("crypt32!CryptStringToBinaryA error getting flags param");
    ...
    println!("{}** {} crypt32!CryptStringToBinaryA str: 0x{:x} len: {} ptr: {} len: {} {}{}", emu.colors.light_red, emu.pos, 
             string, num_chars, ptr, inout_sz, dflags, emu.colors.nc);
